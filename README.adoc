...
6. Annotation `@Table` is not required in model, because table name in 99% of all tables equals of model class name

7. JsonNode field in model automatically converted to JSONB type of PostgreSQL database and back 

8. Utility class `FastMethodInvoker` is a modern reflection access to object properties, in performance comparable to direct access. 
