...
R2DBC-DSL contains the current version of the Spring Data R2dbc, and each commit to the master of Spring Data R2dbc repository is immediately published there under the new subversion, i.e. all changes in the Spring Data R2dbc master are realtime relevant here.
...
Default language may be setting in:

`spring.r2dbc.dsl.fts-lang`

or get if nothing from:

`Locale.getCurrent()`

In table look field with name `tsv`, but can be choosed in `Dsl.create().fts("ts_vector","web query text")`.
[source,postgresql]
----
