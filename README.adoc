...
* On field with name `version` do not required @Version annotation if it has type `Long`, `Integer`, `Short`, `ZonedDateTime` or `LocalDateTime` for optimistic locking

* On field with @ReadOnly annotation on each update the value getting from previous record of database

* On field with @Equality annotation on each update the value is comparing with previous value from database
