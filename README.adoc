...
* Convert First Level Cache to Second Level Cache by property in application.yml: `spring.r2dbc.dsl.second-cache = true` then subscribe to changes from database

* Support Second Level Cache from any Spring Cache implementation and starting with @EnableCaching annotation in Application entry point class
...
after turn on it the all repositories is subscribed to listen database table for any changes and after receive event is evicted repository cache.
...
We can manage for each R2dbcRepository cache custom timeouts and max size:

[source,yaml]
----
spring.r2dbc.dsl.cache:
  <r2dbc repository class simple name>.expireAfterAccess: 500
  <r2dbc repository class simple name>.expireAfterWrite: 1000
  <r2dbc repository class simple name>.maximumSize: 10000
----
