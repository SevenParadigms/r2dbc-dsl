...
* Field with name `version` do not required @Version annotation if it has type `Long`, `Integer`, `Short`, `ZonedDateTime`, `OffsetDateTime` or `LocalDateTime` for optimistic locking
...
* Field with @Now annotation and type `ZonedDateTime`, `OffsetDateTime` or `LocalDateTime` annotation on each update is setting to current timestamp

* Field with name `createdAt` on update using @ReadOnly annotation logic

* Field with name `updatedAt` on update using @Now annotation logic
