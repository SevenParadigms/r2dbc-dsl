...
7. JsonNode type in Java-model automatically converted to JSONB type of PostgreSQL and back

8. Repository `listener` method can subscribe to listen of all changes of table from database in realtime over `Flux<Notification>`
...
11. Utility class `Beans` is a powerfull Spring beans usage conversation with cache: can get beans, dynamic register beans and get properties

12. Transaction manager is enabled by default connection
...
for example column `shops.type` and `column->'header'->>'title'` in result is mapped to class fields `type` and `title`.
