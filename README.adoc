...
* Utility class `R2dbcUtils` can create repository from url: `R2dbcUtils.getRepository("r2dbc:postgresql://postgres:postgres@localhost/abac_rules", AbacRepository::class)` and have `batchSave` massive operation for any type objects in collection. Also have `fun saveBatch(models: Iterable<T>): Flux<Integer>` method for call native Postgres driver batch operation for massive insert as one sql.
